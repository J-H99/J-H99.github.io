...
const introSlides = [
  slide(
    1,
    "intro",
    "클라우드 스토리지는 인터넷 너머의 개인 창고처럼 시작한다",
  ...
  slide(
    ...
    table(
      ...
      ["구분", "서버 디스크", "클라우드 스토리지"],
      [
        ["확장", "서버 용량에 묶임", "서비스 단위 확장"],
        ["장애", "직접 복구", "복제와 내구성 활용"],
        ["배포", "파일 유실 위험", "앱과 데이터 분리"],
        ["권한", "앱에서 구현", "정책과 토큰 사용"]
      ]
    ),
  ...
  slide(
    3,
    "intro",
    "업로드는 파일을 보내는 동작이 아니라 신뢰 경계를 넘기는 동작이다",
  ...
  slide(
    4,
    "intro",
    "다운로드와 공유는 링크 하나처럼 보이지만 권한 계약이다",
    ...
    kv("Access questions", [
      ["Who", "사용자, 서비스 계정, 외부 공유자"],
      ["What", "객체 key와 version"],
      ["How long", "만료 시간"],
      ["From where", "CDN, VPC, public internet"],
      ["Audit", "누가 읽었는가"]
    ]),
  ...
  slide(
    5,
    "intro",
    "클라우드 스토리지에서 폴더는 대부분 진짜 폴더가 아니다",
    ...
    cards("Folder illusion", [
      ["UI", "폴더처럼 표시"],
      ["API", "bucket + key 문자열"],
      ["Prefix", "목록 조회 기준"],
      ["Rename", "복사와 삭제로 구현될 수 있음"]
    ]),
  ...
  slide(
    6,
    "intro",
    "클라우드 스토리지의 네 가지 기본 동사는 upload, store, access, delete다",
    ...
    matrix("Four verbs", [
      ["Upload", "입력 검증과 권한 위임"],
      ["Store", "key, metadata, class"],
      ["Access", "인증, CDN, signed URL"],
      ["Delete", "retention, version, privacy"]
    ]),
    "Provider object storage overview docs",
    ["aws", "azureIntro", "gcsIntro"]
  ),
  ...
  slide(
    7,
    "intro",
    "컴공 학생이 알아야 할 첫 질문은 '파일이 어디 있나'가 아니다",
    ...
    kv("CS questions", [
      ["Address", "어떤 key와 URL인가"],
      ["Ownership", "누가 책임지는가"],
      ["Failure", "무엇이 깨질 수 있는가"],
      ["Cost", "무엇에 돈이 드는가"],
      ["Deletion", "정말 지워졌는가"]
    ]),
  ...
  slide(
    ...
    table(
      ...
      ["개념", "AWS", "Azure", "Google"],
      [
        ["큰 경계", "Account/Bucket", "Storage Account/Container", "Project/Bucket"],
        ["데이터", "Object", "Blob", "Object"],
        ["권한", "IAM/Policy", "RBAC/SAS", "IAM/UBLA"],
        ["이벤트", "SQS/Lambda", "Event Grid", "Pub/Sub"]
      ]
    ),
    "AWS S3 User Guide; Azure Blob Storage introduction; Google Cloud Storage overview",
    ["aws", "azureIntro", "gcsIntro"]
  ...
  slide(
    ...
    cards("Workloads", [
      ["Images", "CDN + derivatives"],
      ["Video", "large upload + streaming"],
      ["Logs", "partition + retention"],
      ["Backup", "immutability + restore"],
  ...
  slide(
    ...
    flow("Learning ladder", [
      ["User", "올리고 받는다"],
      ["Developer", "API와 DB 상태를 맞춘다"],
      ["CS", "분산 시스템 속성을 본다"],
      ["Operator", "비용, 보안, 복구를 운영한다"]
    ]),
    "Provider object storage overview docs",
    ["aws", "azureIntro", "gcsIntro"]
  ),
  ...
  slide(
    11,
    "implementation",
    "웹 서비스에서 파일은 DB row와 객체 key가 함께 움직인다",
    ...
    flow("App storage state", [
      ["DB", "pending row"],
      ["Storage", "object upload"],
      ["Worker", "verify/scan"],
      ["DB", "ready row"],
      ["Cleanup", "orphan delete"]
    ]),
    "Provider object storage overview docs",
    ["aws", "azureIntro", "gcsIntro"]
  ),
  ...
  slide(
    12,
    "implementation",
    "직접 업로드는 서버 부하를 줄이지만 권한 설계가 더 중요해진다",
    ...
    table(
      ...
      ["단계", "역할", "주의점"],
      [
        ["Sign", "앱 서버", "key/size/type 제한"],
        ["Upload", "브라우저", "만료 전 전송"],
        ["Verify", "서버/worker", "size/checksum 검사"],
        ["Publish", "앱", "ready 상태 전환"]
      ]
    ),
  ...
  slide(
    ...
    flow("Object pipeline", [
      ["ObjectCreated", "event"],
      ["Queue", "buffer/retry"],
      ["Worker", "scan/transform"],
      ["DB", "status update"],
      ["CDN", "serve derivative"]
    ]),
  ...
  slide(
    ...
    kv("Static delivery", [
      ["Origin", "객체 스토리지"],
      ["Edge", "CDN cache"],
      ["Versioning", "hash filename"],
      ["Headers", "Cache-Control"],
      ["Invalidation", "덮어쓰기 처리"]
    ]),
  ...
  slide(
    ...
    timeline("Deletion path", [
      ["Request", "사용자 삭제"],
      ["Soft delete", "복구 가능 기간"],
      ["Lifecycle", "만료 정책"],
      ["Audit", "삭제 증적"],
      ["Backup", "복구 범위 확인"]
    ]),
  ...
  slide(
    16,
    "implementation",
    "이제 객체 스토리지를 분산 시스템으로 읽을 준비가 됐다",
    ...
    matrix("Bridge to deep dive", [
      ["Upload", "multipart, checksum"],
      ["Store", "durability, class"],
      ["Access", "IAM, CDN, signed URL"],
      ["Delete", "version, retention"],
      ["Operate", "metrics, cost, DR"]
    ]),
    "AWS, Azure, and Google Cloud official storage docs",
    ["aws", "azureIntro", "gcsIntro"]
  )
...
const deepSlidesBase = [
  slide(
    1,
    "model",
    "클라우드 스토리지는 파일 저장소가 아니라 분산 객체 시스템이다",
    ...
    cards("Object storage surface", [
      ["Data plane", "PUT/GET/LIST/DELETE"],
      ["Control plane", "bucket, policy, lifecycle"],
      ["Metadata", "type, tag, checksum"],
      ["Integration", "event, CDN, analytics"]
    ]),
    "AWS S3 User Guide; Azure Blob Storage introduction; Google Cloud Storage overview",
    ["aws", "azureIntro", "gcsIntro"]
  ),
  ...
  slide(
    ...
    table(
      ...
      ["모델", "접근 단위", "대표 용도"],
      [
        ["Block", "sector / volume", "DB 디스크, VM"],
        ["File", "path / inode", "공유 폴더, NAS"],
        ["Object", "bucket + key", "미디어, 로그, 백업"],
        ["Database", "row / query", "트랜잭션 상태"]
      ]
    ),
  ...
  slide(
    ...
    kv("Object anatomy", [
      ["Bucket", "권한과 정책의 큰 경계"],
      ["Key", "객체 주소와 prefix 인덱스"],
      ["Bytes", "실제 데이터"],
      ["Metadata", "브라우저, 캐시, 처리 상태"],
      ["Policy", "누가 무엇을 할 수 있는가"]
    ]),
    "AWS S3 User Guide; Google Cloud Storage overview",
    ["aws", "gcsIntro"]
  ),
  ...
  slide(
    ...
    matrix("Reliability vocabulary", [
      ["Consistency", "쓰기 직후 무엇을 볼 수 있는가"],
      ["Durability", "저장된 데이터가 사라지지 않는가"],
      ["Availability", "요청을 지금 처리할 수 있는가"],
      ["Recovery", "깨졌을 때 다시 만들 수 있는가"]
    ]),
  ...
  slide(
    ...
    flow("Request planes", [
      ["Client", "PUT/GET"],
      ["Storage API", "data path"],
      ["Admin/IaC", "policy changes"],
  ...
  slide(
    6,
    "model",
    "객체 key 설계는 검색이 아니라 운영을 위한 인덱스 설계다",
    ...
    kv("Key design", [
      ["권장", "tenant/date/type/uuid"],
      ["주의", "email/name/idcard in key"],
      ["정책", "prefix + tag 조건"],
      ["조회", "inventory와 catalog 사용"]
    ]),
    "AWS S3 User Guide; Google Cloud Storage overview",
    ["aws", "gcsIntro"]
  ),
  ...
  slide(
    7,
    "model",
    "대용량 업로드는 단일 요청이 아니라 세션과 청크의 문제다",
    ...
    table(
      "Large upload primitives",
      ["Provider", "Primitive", "운영 포인트"],
      [
        ["AWS", "Multipart upload", "part retry, abort incomplete"],
        ["Azure", "Put Block/List", "block id, commit list"],
        ["Google", "Resumable upload", "session URI, offset"],
        ["공통", "Checksum", "corruption detection"]
      ]
    ),
  ...
  slide(
    8,
    "model",
    "HTTP 캐시와 CDN은 객체 스토리지의 읽기 모델을 바꾼다",
    ...
    flow("Static delivery", [
      ["Object", "origin bytes"],
      ["Metadata", "cache headers"],
      ["CDN", "edge cache"],
      ["Browser", "revalidate or reuse"]
    ]),
    "Provider object storage and CDN integration docs",
    ["aws", "azureIntro", "gcsIntro"]
  ),
  ...
  slide(
    9,
    "aws",
    "S3 bucket은 계정 안의 보안 경계이자 네임스페이스다",
    ...
    matrix("S3 bucket baseline", [
      ["Name", "global namespace"],
      ["Ownership", "object writer vs bucket owner"],
      ["Public access", "account/bucket blocking"],
      ["Audit", "CloudTrail and access logs"]
    ]),
  ...
  slide(
    10,
    "aws",
    "S3 권한은 IAM, bucket policy, access point가 합쳐진 결과다",
    ...
    flow("S3 auth evaluation", [
      ["Principal", "role/session"],
      ["Policy", "IAM + bucket"],
      ["Condition", "prefix/tag/VPC"],
      ["KMS", "key policy"]
    ]),
    "AWS S3 User Guide; S3 security best practices",
    ["aws"]
  ...
  slide(
    11,
    "aws",
    "S3 storage class는 성능, 접근 빈도, 최소 보관 기간의 선택이다",
    ...
    table(
      ...
      ["Class", "적합한 패턴", "주의점"],
      [
        ["Standard", "자주 읽음", "저장비 높음"],
        ["IA", "가끔 읽음", "retrieval 비용"],
        ["Glacier", "장기 보관", "restore 지연"],
        ["Intelligent", "예측 어려움", "monitoring/automation"]
      ]
    ),
  ...
  slide(
    12,
    "aws",
    "S3 Lifecycle은 삭제 자동화가 아니라 상태 전이 자동화다",
    ...
    timeline("Lifecycle states", [
      ["Day 0", "Standard write"],
      ["Day 30", "IA transition"],
      ["Day 180", "Archive transition"],
      ["Day 365", "expire noncurrent"]
    ]),
  ...
  slide(
    13,
    "aws",
    "S3 multipart upload와 checksum은 데이터 무결성 설계의 일부다",
  ...
  slide(
    14,
    "aws",
    "S3 Event Notifications는 후처리 파이프라인의 시작점이다",
    ...
    flow("S3 event pipeline", [
      ["PUT object", "raw upload"],
  ...
  slide(
    15,
    "aws",
    "S3 Replication은 DR뿐 아니라 데이터 거버넌스 기능이다",
    ...
    matrix("Replication decisions", [
      ["Scope", "prefix/tag filter"],
      ["Target", "same or cross region"],
      ["Delete", "delete marker policy"],
      ["Encryption", "KMS key mapping"]
    ]),
  ...
  slide(
    16,
    "aws",
    "Object Lock은 삭제 방지가 아니라 규정 준수 상태 머신이다",
    ...
    table(
      ...
      ["개념", "의미", "주의점"],
      [
        ["Retention", "기간 기반 보존", "mode 차이"],
        ["Legal hold", "사건 기반 보존", "수동 해제"],
        ["Versioning", "불변 객체 단위", "비용 증가"],
        ["Replication", "보존정보 복제", "대상 bucket 조건"]
      ]
    ),
  ...
  slide(
    17,
    "aws",
    "S3 Storage Lens, Inventory, Batch Operations는 운영 규모의 도구다",
    ...
    cards("S3 at scale", [
      ["Storage Lens", "usage and activity visibility"],
      ["Inventory", "object catalog export"],
      ["Batch Ops", "bulk object actions"],
  ...
  slide(
    18,
    "aws",
    "S3를 데이터 레이크로 쓸 때는 파일 포맷과 catalog가 핵심이다",
  ...
  slide(
    19,
    "azure",
    "Azure Storage Account는 Blob만이 아니라 계정 단위 플랫폼이다",
    ...
    kv("Azure hierarchy", [
      ["Storage account", "namespace and settings"],
      ["Container", "blob grouping"],
      ["Blob", "object data"],
  ...
  slide(
    20,
    "azure",
    "Block, Append, Page Blob은 같은 Blob Storage 안의 다른 쓰기 모델이다",
    ...
    table(
      ...
      ["Type", "쓰기 모델", "대표 용도"],
      [
        ["Block", "stage + commit", "파일, 미디어"],
        ["Append", "append-only", "로그"],
        ["Page", "random page IO", "VHD"],
        ["ADLS path", "HNS path ops", "분석 레이크"]
      ]
    ),
  ...
  slide(
    21,
    "azure",
    "Azure access tier는 Hot, Cool, Cold, Archive의 경제 모델이다",
    ...
    timeline("Blob access tiers", [
      ["Hot", "frequent access"],
      ["Cool", "infrequent online"],
      ["Cold", "rare online"],
      ["Archive", "offline rehydrate"]
    ]),
  ...
  slide(
    22,
    "azure",
    "Azure redundancy는 LRS, ZRS, GRS, GZRS, RA 옵션의 조합이다",
    ...
    table(
      ...
      ["옵션", "범위", "읽기"],
      [
        ["LRS", "single region", "primary"],
        ["ZRS", "zones", "primary"],
        ["GRS", "paired region", "failover"],
        ["RA-GRS", "paired region", "secondary read"]
      ]
    ),
  ...
  slide(
    23,
    "azure",
    "Azure 권한은 Entra ID, RBAC, SAS, account key가 공존한다",
    ...
    flow("Azure auth choices", [
      ["Entra ID", "RBAC roles"],
      ["SAS", "delegated URL"],
      ["Account key", "broad secret"],
  ...
  slide(
    24,
    "azure",
    "Private Endpoint와 firewall은 Blob 접근 경로를 네트워크로 제한한다",
    ...
    matrix("Network controls", [
      ["Firewall", "allowed networks"],
  ...
  slide(
    25,
    "azure",
    "Blob soft delete, versioning, immutability는 삭제 사고의 복구 계층이다",
    ...
    table(
      ...
      ["기능", "보호 대상", "운영 고려"],
      [
        ["Soft delete", "deleted blobs", "retention window"],
        ["Versioning", "overwrites", "version cost"],
        ["Immutable policy", "regulated data", "lock state"],
        ["Legal hold", "case hold", "manual lifecycle"]
      ]
    ),
  ...
  slide(
    26,
    "azure",
    "Object replication은 계정 간 비동기 복제 정책이다",
    ...
    flow("Azure replication", [
      ["Source account", "block blobs"],
      ["Rule", "container/prefix"],
      ["Async copy", "lag"],
      ["Destination", "DR or analytics"]
    ]),
  ...
  slide(
    27,
    "azure",
    "Event Grid와 Functions는 Blob 변경을 애플리케이션 이벤트로 바꾼다",
  ...
  slide(
    28,
    "azure",
    "Data Lake Storage Gen2는 Blob Storage에 계층형 namespace를 더한다",
    ...
    matrix("ADLS Gen2", [
      ["HNS", "directory operations"],
      ["ACL", "fine-grained access"],
      ["Analytics", "Spark/Synapse"],
      ["Lifecycle", "tier and retention"]
    ]),
  ...
  slide(
    29,
    "gcp",
    "Google Cloud Storage는 bucket, object, location, storage class로 시작한다",
    ...
    kv("GCS primitives", [
      ["Bucket", "location and policy"],
      ["Object", "data + metadata"],
      ["Generation", "object version identity"],
      ["Class", "cost/access profile"]
    ]),
  ...
  slide(
    30,
    "gcp",
    "Uniform bucket-level access는 GCS 권한 모델을 IAM 중심으로 단순화한다",
    ...
    flow("GCS access model", [
      ["IAM", "bucket-level roles"],
      ["UBLA", "disable object ACL"],
      ["PAP", "block public"],
  ...
  slide(
    31,
    "gcp",
    "GCS signed URL과 signed policy는 제한된 시간의 객체 접근 계약이다",
  ...
  slide(
    32,
    "gcp",
    "GCS storage class와 Autoclass는 접근 패턴을 비용 정책으로 바꾼다",
    ...
    table(
      ...
      [
        ...
        ["Nearline", "monthly", "retrieval cost"],
        ["Coldline", "quarterly", "backup/archive"],
        ["Archive", "rare", "long-term retention"],
        ["Autoclass", "unknown", "automatic transitions"]
      ]
    ),
  ...
  slide(
    33,
    "gcp",
    "GCS Lifecycle rule은 age, version, class, prefix 조건으로 객체를 전이한다",
    ...
    timeline("GCS lifecycle", [
      ["Upload", "Standard"],
      ["30 days", "Nearline"],
      ["180 days", "Archive"],
      ["Policy end", "delete or retain"]
    ]),
  ...
  slide(
    34,
    "gcp",
    "Object Versioning, retention policy, hold는 GCS의 데이터 보호 계층이다",
    ...
    table(
      ...
      ["기능", "대상", "설계 포인트"],
      [
        ["Versioning", "generations", "cleanup rule"],
        ["Retention", "time policy", "bucket lock"],
        ["Holds", "workflow/case", "release process"],
        ["Precondition", "generation match", "race prevention"]
      ]
    ),
  ...
  slide(
    35,
    "gcp",
    "Pub/Sub notifications는 GCS 변경을 이벤트 스트림으로 노출한다",
    ...
    flow("GCS event path", [
      ["Object finalize", "new generation"],
      ["Pub/Sub", "delivery"],
      ["Worker", "process"],
      ["State", "indexed/published"]
    ]),
  ...
  slide(
    36,
    "gcp",
    "GCS resumable upload와 compose는 대용량 객체 처리의 핵심 도구다",
    ...
    flow("GCS large object", [
      ["Start session", "resumable URI"],
      ["Upload chunks", "offset retry"],
      ["Verify", "CRC32C/MD5"],
      ["Publish", "DB state"]
    ]),
  ...
  slide(
    37,
    "gcp",
    "Storage Insights와 Inventory 계열 데이터는 bucket 운영을 분석 문제로 만든다",
    ...
    cards("GCS operations data", [
      ["Insights", "environment visibility"],
      ["Usage logs", "access and lifecycle"],
      ["BigQuery", "query patterns"],
      ["Lifecycle", "policy feedback"]
    ]),
  ...
  slide(
    38,
    "gcp",
    "BigQuery, Dataflow, GKE와 연결될 때 GCS는 데이터 플랫폼의 landing zone이 된다",
    ...
    matrix("GCS as platform", [
      ["BigQuery", "load/external data"],
      ["Dataflow", "staging and output"],
      ["GKE", "artifact/log"],
      ["ML", "dataset/model lineage"]
    ]),
  ...
  slide(
    39,
    "architecture",
    "직접 업로드 아키텍처는 권한 위임과 상태 머신의 조합이다",
    ...
    flow("Direct upload state", [
      ["Request", "auth and quota"],
      ["Upload", "temporary grant"],
      ["Process", "scan/resize"],
      ["Publish", "DB visible"]
    ]),
  ...
  slide(
    40,
    "architecture",
    "멀티테넌트 저장소는 prefix, tag, policy, KMS key 경계를 맞춰야 한다",
    ...
    table(
      ...
      ["전략", "장점", "위험"],
      [
        ["Bucket per tenant", "strong boundary", "many resources"],
        ["Prefix per tenant", "simple fleet", "policy complexity"],
        ["Tag boundary", "analytics/cost", "not hard isolation"],
        ["Key per tenant", "crypto isolation", "key ops burden"]
      ]
    ),
  ...
  slide(
    41,
    "architecture",
    "CDN 앞의 객체 스토리지는 immutable asset pipeline으로 설계한다",
  ...
  slide(
    42,
    "architecture",
    "보안 사고는 public bucket 하나가 아니라 제어 경계 실패로 발생한다",
    ...
    matrix("Threat model", [
      ["Exposure", "public or leaked URL"],
      ["Tamper", "overwrite/delete"],
      ["Exfiltration", "bulk download"],
      ["Lockout", "KMS/IAM denial"],
      ["Poisoning", "malicious upload"],
      ["Cost attack", "egress/request spike"]
    ]),
  ...
  slide(
    ...
    table(
      ...
      ["모델", "장점", "주의점"],
      [
        ["Provider key", "low ops", "less control"],
        ["CMK", "audit/control", "policy complexity"],
        ["Per-tenant key", "isolation", "key sprawl"],
        ["Client-side", "max control", "search/process limits"]
      ]
    ),
  ...
  slide(
    44,
    "architecture",
    "관측성은 bytes 저장량보다 요청 모양과 정책 효과를 본다",
    ...
    cards("Storage SLO signals", [
      ["Availability", "request success"],
      ["Latency", "p95/p99 read"],
      ["Cost", "egress/request/retrieval"],
      ["Protection", "versioning/lock coverage"],
      ["Drift", "public/policy changes"],
      ["Pipeline", "event backlog"]
    ]),
  ...
  slide(
    45,
    "architecture",
    "비용 모델은 저장 GB보다 요청, 송신, 복구, 최소 보관 기간이 더 어렵다",
    ...
    table(
      ...
      ["항목", "증가 원인", "완화 방법"],
      [
        ["Capacity", "old data", "lifecycle/retention"],
        ["Requests", "small objects", "batch/cache"],
        ["Egress", "internet/cross-region", "CDN/locality"],
        ["Retrieval", "archive reads", "restore planning"],
        ["Replication", "DR copies", "scope filters"]
      ]
    ),
  ...
  slide(
    ...
    flow("Recovery drill", [
      ["Detect", "incident scope"],
      ["Isolate", "credentials/policy"],
      ["Restore", "objects + metadata"],
      ["Verify", "application reads"]
    ]),
  ...
  slide(
    47,
    "architecture",
    "AWS, Azure, Google의 차이는 이름보다 기본 경계와 통합 방식에서 나온다",
    ...
    table(
      "Provider comparison",
      ["축", "AWS S3", "Azure Blob", "GCS"],
      [
        ["큰 경계", "Account/Bucket", "Storage Account", "Project/Bucket"],
        ["권한", "IAM + bucket policy", "Entra RBAC + SAS", "IAM + UBLA"],
        ["이벤트", "SQS/Lambda/EventBridge", "Event Grid/Functions", "Pub/Sub"],
        ["분석", "Athena/Glue", "ADLS/Synapse", "BigQuery/Dataflow"]
      ]
    ),
    "AWS, Azure, and Google Cloud official storage docs",
    ["aws", "azureIntro", "gcsIntro"]
  ...
  slide(
    48,
    "architecture",
    "최종 설계는 객체 하나가 아니라 데이터 제품의 수명주기를 다룬다",
    ...
    matrix("Design checklist", [
      ["Object model", "key, metadata, version"],
      ["Access", "IAM, token, network"],
      ["Pipeline", "event, scan, transform"],
      ["Cost", "class, request, egress"],
      ["Protection", "version, lock, backup"],
      ["Operations", "metrics, audit, drill"]
    ]),
...
const diagramOverrides = {
  ...
  2: diagram(
    "Server disk vs storage service",
    [
  ...
  5: diagram(
    "Folder illusion over object keys",
    [
  ...
  14: diagram(
    "CDN and immutable asset delivery",
    [
  ...
  35: diagram(
    "Azure storage account hierarchy",
    [
...
function writeInteractive() {
  ...
  html = html
    ...
    .replace(
      /<title>클라우드 스토리지[^<]*<\/title>/,
      "<title>클라우드 스토리지 입문부터 심화까지</title>"
    )
    ...
    .replace(
      ...
      "slide.no === 1 || slide.no === slides.length"
    )
    .replace(
      /slideCount\.textContent = `\$\{String\(slide\.no\)\.padStart\(2, "0"\)\} \/ 30`;/,
      'slideCount.textContent = `${String(slide.no).padStart(2, "0")} / ${slides.length}`;'
    )
    .replace(
      /progressLabel\.textContent = `\$\{String\(slide\.no\)\.padStart\(2, "0"\)\} \/ 30`;/,
    ...
    .replace(
      '<div class="field-note">${escapeHtml(slide.field)}</div>',
      '<div class="field-note"><span class="field-label">상세 해설</span>${escapeHtml(slide.field)}</div>'
    )
    .replace(
      /<span class="field-label">심화 해설<\/span>/g,
      '<span class="field-label">상세 해설</span>'
    );
...
function writeNotes() {
  ...
  const toc = slides
    .map(
      (item) =>
        `<li><a href="#${slug(`${item.no} ${item.title}`)}">${String(item.no).padStart(2, "0")}. ${escapeHtml(item.title)}</a></li>`
    )
    .join("");
